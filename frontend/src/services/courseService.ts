...
function normalizeLevel(raw: string): "Beginner" | "Intermediate" | "Advanced" {
...
function mapCourse(bc: BackendCourse, lectures?: BackendLecture[]): Course {
  ...
  const instructorBio =
    typeof bc.instructor === "string" ? "" : (bc.instructor?.bio ?? "");
  const instructorAvatar =
    typeof bc.instructor === "string" ? "" : (bc.instructor?.avatar ?? "");
  ...
  return {
    ...
    enrolledStudents:
      bc.enrolledStudents?.map((s) => ({
        student:
          typeof s.student === "object" && s.student
            ? (s.student as any)._id || (s.student as any).id
            : s.student,
        rating: s.rating,
      })) ?? [],
  ...
  try {
    ...
    return backendData.map((data) => {
      ...
      const course = mapCourse(
        data.course,
        Array.isArray(data.course.lectures)
          ? (data.course.lectures as unknown as BackendLecture[])
          : undefined,
      );
      const firstLecture =
        Array.isArray(data.course.lectures) && data.course.lectures.length > 0
 ...
 */
export async function getProcessingLectures(
  courseId: string,
